function y = withoutNoise(M)
%to implement oversampled uniform quantization
Xm = 10;
...
for B = 1:15
    ...
    %input3 = upsample(inputRaw, M);
     for i = 1:length(inputRaw)
          xHat(i) = Quantizer(inputRaw(i), Xm, B);
     end
    xHat;
    signal1 = inputRaw;
...
%     figure(9);plot(abs(fft(noise3)));title('noise3');
    ...
    SNRTheory(B) = 6.02*B+10.8-20*log10(Xm/sqrt(varInput(B)))+10*log10(M);
end
bits = 2:16;
...
str = sprintf('Oversampled Uniform Quantization M= %u', M);
